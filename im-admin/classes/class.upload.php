...
class upload {
    ...
    function process($server_path) {
        ...
        if ($this->processed) {
			...
			// Peter young changed this to start file numbering from _1
			
                $this->file_dst_name_body = $this->file_new_name_body.'_1';
            ...
            if (!$this->file_auto_rename) {
				...
				//Peter Young changed $cpt to continue numbering files from _1
                $cpt = 2;
                while (@file_exists($this->file_dst_pathname)) {
				
				//Peter Young hack to take out the 1 of the first file: only works with less that 10 files
				
                    $this->file_dst_name_body = str_replace('_1','', $body) .'_'. $cpt;
                    $this->file_dst_name = $this->file_dst_name_body . (!empty($this->file_dst_name_ext) ? '.' . $this->file_dst_name_ext : '');
                    $cpt++;
                    $this->file_dst_pathname = $this->file_dst_path . $this->file_dst_name;
                }               
