		...
		/*----------------------------------------------------------------------*/
			...
			$content.find('ul.gallery').wl_Gallery({
				...
				onDelete: function(el, href, title){
					if(href){
						$.confirm('Borrar Imágen?',function(){
																		
						$.ajax({
						type: "GET",
						url: "image_delete.php",
						data: ("photoid="+ $(el).attr("id")),
				
					success: function(data) {
						if (data == 'success') {
							$('#msg').html('Imágen Borrado')
													} 
					},
					error: function () {
						if(data=='No ID set!') {
						$('#msg').html('Imágen No Borrado');
						}
					}
