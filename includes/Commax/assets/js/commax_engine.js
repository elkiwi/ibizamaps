$(document).ready(function(){
	...
	var commaxPostUrl	= "http://newmaps-net/includes/Commax/";
	
	// First get all the configuration variables from comment.class.php
	$.post(commaxPostUrl+'include/commax.processor.php?type=getConfig',function(data) {
		// config vars
	...
	***********************************************/
	...
	$('#addCommentForm').live('submit', function(e){
		...
		$.post(commaxPostUrl+'include/commax.processor.php?type=comment',$(this).serialize(),function(msg){
			...
			if(msg.status){
				...
				if (theDisplayOrder == "DESC") {
					if ($(".comment:first").length != 0) {
						...
						$("#commax_container").find("p:first").remove();
					...
					if ($(".commax_bottom_paginator").length != 0) {
						$(msg.html).hide().insertBefore('.commax_bottom_paginator').slideDown();
					} else {
						$(msg.html).hide().insertBefore('#addCommentContainer').slideDown();
					}
				}
				...
				$(".commax_captcha_img").attr("src", commaxPostUrl+'include/commax_captcha.php?'+Math.random()).animate({ opacity: 1 }, 500);
	...
	$('.addReplyForm').live('submit', function(e){
		...
		// Sending the form fileds to commax.processor.php:
		$.post(commaxPostUrl+'include/commax.processor.php?type=reply&id='+parentID+'',$(this).serialize(),function(msg){
	...
	***********************************************/
	...
	$('.really_remove_btn').live('click', function(e) {
		...
		$.post(commaxPostUrl+'include/commax.processor.php?type=rem&id='+parentID+'', function(msg){
			if(msg.status){
	...
	$('.really_aprove_btn').live('click', function(e) {
		...
		parentID = parentID.substring(8);
		// Sending the request to commax.processor.php:
		$.post(commaxPostUrl+'include/commax.processor.php?type=app&id='+parentID+'', function(msg){
			if(msg.status){
	...
	***********************************************/
	...
	ajaxPage_ref_id = $("input#ref_id_value").val();
	
	// Loading info show function
	function loading_show() {
		$('#commax_container').html('<div class="loading_content">'+L_LOADING+'</div>').hide().fadeIn('fast');
	}
	...
	function loadData(ajaxPage_ref_id, page, ipp) {
		$.ajax({
			...
			success: function(msg) {
				$("#commax_container").html(msg).hide().fadeIn(500,function(){
					activate_replies();
				});
				activate_forms();
			}
	...
	$('.commax_paginator > a').live('click',function(e){
		e.preventDefault();
	...
	***********************************************/
	...
	$('.commax_change_image_comm').live('click',function(e){
		e.preventDefault();
		$('.commax_captcha_img_comm').animate({
			opacity: 0.25
		}, 20., function() {
			$(".commax_captcha_img").attr("src", commaxPostUrl+'include/commax_captcha.php?'+Math.random()).animate({ opacity: 1 }, 500);
			$('#commax_anti_bot_input').focus();
		});
	});
	
	
	// Reply Change captcha image button listener
	$('.commax_change_image_repl').live('click',function(e){
		e.preventDefault();
		parentID 	= $(this).attr("id");
		parentID 	= parentID.substring(8);
		$('.cci_rpl_'+parentID).animate({
			...
			$(".commax_captcha_img").attr("src", commaxPostUrl+'include/commax_captcha.php?'+Math.random()).animate({ opacity: 1 }, 500);
			$('#commax_anti_bot_input_'+parentID).focus();
		});
	});
	...
	***********************************************/
	...
	function karmaSetVote(ID, voteUPorDOWN) {
		$.post(commaxPostUrl+'include/commax.processor.php?type=karma_vote&ID='+ID+'&voteUPorDOWN='+voteUPorDOWN, function(data) {
			working = false;
			if (data.status) {
