...
class Ccommax {
	...
	*/
	private function recursive_build_comment($ref_id, $parent_id = "-") {
		...
		// Get the comments
		$result 	= mysql_query("SELECT * FROM commax_comments WHERE ref_id = '$ref_id' AND parent = '$parent_id' $and_show_it ORDER BY id ".$this->display_order." $limit");
	...
	*/
	public function commentHasReplies($id) {
		$result 	= mysql_query("SELECT id FROM commax_comments WHERE parent = '$id'");
	...
	*/
	public function numberComments($id) {
		...
		$result = mysql_query("SELECT id FROM commax_comments WHERE ref_id = '$id' AND parent = '-' $and_show_it ");
	...
	*/
	private function karma_increaseVote($ID) {
		...
		mysql_query("UPDATE commax_comments SET up_vote = up_vote + 1 WHERE id = '$ID'");
		// get the initial karma value, before adding new vote
		$result 		= mysql_query("SELECT karma FROM commax_comments where id = '$ID'") or die("error db - com.class #" . __line__);
		$row 			= mysql_fetch_object($result);
		$intial_karma 	= $row->karma;
		// adds one more vote
		$final_karma	= $intial_karma + 1;
		// updates karma with new vote
		mysql_query("UPDATE commax_comments SET karma = '$final_karma' WHERE id = '$ID'");
	...
	*/
	private function karma_decreaseVote($ID) {
		...
		mysql_query("UPDATE commax_comments SET dw_vote = dw_vote + 1 WHERE id = '$ID'");
		// get the initial karma value, before adding new vote
		$result 		= mysql_query("SELECT karma FROM commax_comments where id = '$ID'") or die("error db - com.class #" . __line__);
		$row 			= mysql_fetch_object($result);
		$intial_karma 	= $row->karma;
	...
	*/
	private function karma_checkAlreadyVoted($ID) {
		...
		// Oh well... voting only for registered users.
			...
			$result = mysql_query("SELECT id FROM commax_karma_voted WHERE com_id = '$ID' AND email = '$email'");
			...
			mysql_query("INSERT INTO commax_karma_voted(id, com_id, email) VALUES ('', '$ID', '$email')");
			// return false (User has not voted yet
			return false;
		}
	}
	...
	*/
	public function insert_new_comment(&$arr) {
		...
		mysql_query("INSERT INTO commax_comments(ref_id, parent, name, url, email, body, show_it)
						VALUES (
							'".$arr['ref_id']."',
							'".$arr['parent']."',
							'".$arr['name']."',
	...
	*/
	public function getConfigurations($wich = "all") {
		...
		// get result
		$result = mysql_query("SELECT * FROM commax_config $where_clause ORDER BY id ASC");
	...
	*/
	public function removeComment($id) {
		...
		mysql_query("DELETE FROM commax_comments WHERE id = '$id'");
	...
	*/
	public function approveComment($id) {
		...
		// change status in db
		mysql_query("UPDATE commax_comments SET show_it = '1' WHERE id = '$id'");
	...
	*/
	public function saveConfiguration($wich, $what) {
		return mysql_query("UPDATE commax_config SET cfg_value = '$what' WHERE cfg_name = '$wich'") or die("HMMMM");
	}
	...
	*/
	public function adminGetMainComments() {
		$result = mysql_query("SELECT ref_id, COUNT(id) AS total FROM commax_comments GROUP BY ref_id");
		return $result;
	}
	...
	*/
	public function adminGetUnreviewdTotalComments($ref_id) {
		$result = mysql_query("SELECT COUNT(id) AS total FROM commax_comments WHERE ref_id = '$ref_id' AND show_it = '0'");
		while($row = mysql_fetch_assoc($result)) {
			$out = $row["total"];
		}
		return $out;
	...
	*/
	private function recursive_build_unreviewed_comment($ref_id) {
		...
		$result 	= mysql_query("SELECT * FROM commax_comments WHERE ref_id = '$ref_id' $and_show_it ORDER BY id ASC");
		$out 		= "";
	...
	*/
	public function admin_approveComment($id) {
		if (mysql_query("UPDATE commax_comments SET show_it = '1' WHERE id = '$id'"))
			return "1";
		else
	...
	*/
	public function admin_removeComment($id) {
		...
		else if (mysql_query("DELETE FROM commax_comments WHERE id = '$id'"))
			return "1";
