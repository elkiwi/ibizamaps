...
/**
*
* This is the main commax class file. It handles all the render and operations.
* This calss is used to frontend comment system display. Also processes the AJAX request
* that comes from the commax.processor.php file. It also processes the admin panel requests
*
* @author 	Eduardo Pereira
* @date		03/11/2011
* @website	http://www.voindo.eu/commax
*
* @requirements PHP 5.2+ with GD2 library support.
*
* @package commax.class
*
*/
...
class Ccommax {
	...
	private $commax_user_name;
	private $commax_user_email;
	...
	public function __construct() {
		...
		*********************************************************************/
		...
		// $this->db_host 		= $YOUR_db_host_VAR;
		// $this->db_user 		= $YOUR_db_user_VAR;
		// $this->db_pass 		= $YOUR_db_pass_VAR;
		// $this->db_database 	= $YOUR_db_database_VAR;

		// THE DATABASE MANUAL CONFIG
		/*$this->db_host 			= "localhost";
		$this->db_user 			= "root";
		$this->db_pass 			= "root";
		$this->db_database 		= "maps_res_2012";
		*/

		$this->db_host 			= "localhost";
		$this->db_user 			= "wwwibiz_kiwi2013";
		$this->db_pass 			= "!kiwi4796";
		$this->db_database 		= "wwwibiza_2013maps";
		/*************************************************************
		...
		**************************************************************/
		...
		$link = mysqli_connect($this->db_host, $this->db_user, $this->db_pass) or die('Unable to establish a DB connection');
		// Set UTF-8 mode
		mysqli_query("SET NAMES 'utf8'");
		// Selects the database
		mysqli_select_db($this->db_database, $link);

		// Now let's look for session vars. If not present, default them...
		if ((isset($_SESSION["commax_moderated_com"])) and ($_SESSION["commax_moderated_com"] === true))
			$session_commax_moderated_com = true;
		else
			$session_commax_moderated_com = false;

		// Now let's look for session vars. If not present, default them...
		if ((isset($_SESSION["commax_comments_closed"])) and ($_SESSION["commax_comments_closed"] === true))
			$session_commax_comments_closed = true;
		else
			$session_commax_comments_closed = false;

		// Now let's look for session vars. If not present, default them...
		if ((isset($_SESSION["commax_admin_viewing"])) and ($_SESSION["commax_admin_viewing"] === true))
			$session_commax_admin_viewing = true;
		else
			$session_commax_admin_viewing = false;

		// Now let's look for session vars. If not present, default them...
		if ((isset($_SESSION["commax_is_user_logged"])) and ($_SESSION["commax_is_user_logged"] === true))
			$session_commax_is_user_logged = true;
		else
			$session_commax_is_user_logged = false;

		// Now let's look for session vars. If not present, default them...
		if ((isset($_SESSION["commax_user_name"])) and ($_SESSION["commax_user_name"] !== ""))
			$session_commax_user_name = $_SESSION["commax_user_name"];
		else
			$session_commax_user_name = "";

		// Now let's look for session vars. If not present, default them...
		if ((isset($_SESSION["commax_user_email"])) and ($_SESSION["commax_user_email"] !== ""))
			$session_commax_user_email = $_SESSION["commax_user_email"];
		else
			$session_commax_user_email = "";

		// Now let's look for session vars. If not present, default them...
		if ((isset($_SESSION["commax_language"])) and ($_SESSION["commax_language"] !== ""))
			$session_commax_language = $_SESSION["commax_language"];
		else
			$session_commax_language = "en";
		...
		*************************************************************************/
		...
		// We define here BASE_PATH wich is the ABSOLUTE PATH of this commax.class.php file.
		define('BASE_PATH',str_replace('\\','/',dirname(__FILE__)));

		// Include the purifier class. If the purifier folder is in same folder as this commax.class.php file, next line should be fine because we set up earlier BASE_PATH
		// Remeber to test with error reporting ON, as require_once will throw fatal error if class not found.
		require_once BASE_PATH . '/htmlpurifier/HTMLPurifier.auto.php';

		// GENERAL MANUAL CONFIG
		$this->page_url			= "http://newmaps-net/includes/Commax/";						// Full page URL (Used to access images for gravatar and smiles) ex.g. http://www.yoursite.com/ OR http://127.0.0.1/Commax/
		$this->commax_url		= "/includes/Commax/";												// Location of commax foder (ATTENTION $this->page_url RELATIVE) ex.g. commax/
		$this->image_folder		= "assets/img/";									// Location of images foder (ATTENTION $this->page_url RELATIVE) ex.g. assets/img/
		$this->smiles_folder	= "assets/img/emoticons/";							// Location of smiles foder (ATTENTION $this->page_url RELATIVE) ex.g. assets/img/emoticons/
		$this->lang_folder		= BASE_PATH . "/locale/";							// Location of translation folder (ATTENTION BASE_PATH relative) ex.g. /locale/

		// GENERAL SESSION CONFIG
		$this->closed_comments	= $session_commax_comments_closed;					// THIS IS SESSION CONFIGURABLE (Per post id comments closed flag)
		$this->mod_comment		= $session_commax_moderated_com;					// THIS IS SESSION CONFIGURABLE (Per post id comments moderation flag. If TRUE will override the main mod_comments FALSE)
		$this->admin_viewing	= $session_commax_admin_viewing;					// THIS IS SESSION CONFIGURABLE (Set to true if admin is viewing comments. Activate admin tools)
		$this->is_user_logged	= $session_commax_is_user_logged;					// THIS IS SESSION CONFIGURABLE (Set to true if User is Logged In - Only used when $this->reg_users_only is TRUE)
		$this->commax_user_name	= $session_commax_user_name;						// THIS IS SESSION CONFIGURABLE (Get the logged in user name FROM the session var)
		$this->commax_user_email= $session_commax_user_email;						// THIS IS SESSION CONFIGURABLE (Get the logged in user email FROM the session var)
		$this->current_lang		= $session_commax_language;							// THIS IS SESSION CONFIGURABLE (Get the desired language - default "en" English)
	...
	*
	* This function is only called by Ajax Call from commax_engine.js
	* commax_engine.js will get it's configurations vars from here.
	...
	*
	*/
	public function build_comment_system($ref_id) {

		// let's start, first check number of total comments and assign to var
		$this->items_total = $this->numberComments($ref_id);

		// Start output var. And start with a hidden input containing the $ref_id for the ajax pagination to know what are we showing
		$out = '<input type="hidden" id="ref_id_value" value="'.$ref_id.'" />';
		...
		if ($this->items_total > 0) {
			...
			if ($this->default_ipp != "ALL") {
				...
				$this->paginate();
				// output the top pagination bar with the class commax_top_paginator
				$out .= $this->display_pages("commax_top_paginator");
			}

			// output start the recursive build comments loop
			$out .= $this->recursive_build_comment($ref_id);

			// again if we do no have a show all option on, the show bottom pagination with class commax_bottom_paginator
			if ($this->default_ipp != "ALL")
				$out .= $this->display_pages("commax_bottom_paginator");


			// comments are close?
			if ($this->closed_comments == false) {
				// NO comments are not closed. So is commax only to registered users?
				if ($this->reg_users_only == true) {
					...
					if ($this->is_user_logged == true) {
						...
						$out .= $this->comment_form_user_logged_in_XHTML($ref_id);
					} else {
						// NO user not logged in. So show comments only to registered users form
						$out .= $this->comment_only_register_XHTML($ref_id);
					}
				} else {
					$out .= $this->comment_form_XHTML($ref_id);
				}
			} else {
				// YES comments closed, so show closed comments form
				$out .= $this->comment_closed_XHTML($ref_id);
			}


		} else {
			// so no comments. Show message and next the comment form
			$out .= $this->noCommentsYet_XHTML($ref_id);
	...
	*/
	public function commentHasReplies($id) {
		$result 	= mysqli_query("SELECT id FROM commax_comments WHERE parent = '$id'");
	...
	*
	*/
	public function numberComments($id) {
		// Let's check if it's admin viewing to show all comments, or not and show only published comments
		if ($this->admin_viewing === true)
			$and_show_it = "";
		else
		...
		$result = mysqli_query("SELECT id FROM commax_comments WHERE ref_id = '$id' AND parent = '-' $and_show_it ");

		// Sets the variable with number of comments
		$this->nbComments = mysqli_num_rows($result);
	...
	*/
	public function pre_Markup($replies = false, $adding_new = false) {
		...
		$url = $this->page_url . '/' . $this->image_folder . 'default_avatar.gif';
		...
		if ($replies === true) {
			...
			if (($this->reg_users_only == true) and ($this->is_user_logged === true))
				$replyFormXHTML = $this->reply_form_user_logged_in_XHTML($d["id"], $d["ref_id"]);
			else
				$replyFormXHTML = $this->reply_form_XHTML($d["id"], $d["ref_id"]);
		...
		$adminTools = '';
		if ($this->admin_viewing === true) {
			$adminTools = '<a class="remove_btn" id="rem_btn_'.$d["id"].'" href="#">'.$this->__L("REMOVE").'</a>';
		}
		...
		if ($this->admin_viewing_backend === true) {
			...
			$adminTools 	= '
								<form name="form_remove_comm" class="form_remove_comm" method="post">
									...
									<input type="submit" class="remove_btn" value="'.$this->__L("REMOVE").'" />
								</form>
							';
			if ((isset($d["show_it"])) and ($d["show_it"] == "0"))
				$adminTools    .= '
									<form name="form_approve_comm" class="form_approve_comm" method="post">
										...
										<input type="submit" class="aprove_btn" value="'.$this->__L("APPROVE").'" />
									</form>
								';
		}
	...
	*/
	private function comment_XHTML($id, $replyClass, $link_open, $md5email, $urleUrl, $link_close, $name, $body, $date1, $timeago, $replyLink, $replyFormXHTML, $adminTools, $special_hidden_class, $says, $karmaStuff) {
		...
		$out	='
				<div class="comment id_'.$id.' '.$replyClass.' '.$special_hidden_class.'">
					<div class="com_wrapper">
						<a name="comment_'.$id.'"></a>
						<div class="avatar">
							'.$link_open.'
								<img src="http://www.gravatar.com/avatar/'.$md5email.'?size=50&amp;default='.$urleUrl.'" />
							'.$link_close.'
						</div>
						...
						<div class="btn_tools" id="tools_'.$id.'">
							<div class="date" title="Added at '.$date1.'">'.$timeago.'</div>
							'.$karmaStuff.'
							'.$replyLink.'
							<div class="admin_tools">
								'.$adminTools.'
							</div>
							<div class="commax_clear"></div>
						</div>
						<div class="commax_clear"></div>
	...
	*
	*/
	private function noCommentsYet_XHTML($ref_id) {
	...
	*
	*/
	public function comment_form_XHTML($ref_id) {
		$out = '
			...
			<div style="display: none;" class="yes_javascript_yes_form">
				...
				<form id="addCommentForm" method="post" action="">
					...
					<div>
						...
						<label for="url">'.$this->__L("YOURSITE").'</label>
						<input type="text" name="url" id="url" />
						<label for="body">'.$this->__L("YOURCOMM").'</label>
		...
		if ($this->captcha_enabled == true) {
			$out .= '
						<img src="'.$this->commax_url.'include/commax_captcha.php" class="commax_captcha_img commax_captcha_img_comm" />
						<a href="#" class="commax_change_image_comm">'.$this->__L("CHANGECAPTCHA").'</a>
						<label for="commax_anti_bot_input">'.$this->__L("INSERTCAPTCHA").'</label>
	...
	/**
	* Function comment_form_user_logged_in_XHTML()
	...
	*
	*/
	public function comment_form_user_logged_in_XHTML($ref_id) {
		$out = '
		<div id="addCommentContainer">
			<div class="no_javascript_no_form">'.$this->__L("NOJSNOCOMM").'</div>
			<div style="display: none;" class="yes_javascript_yes_form">
				...
				<form id="addCommentForm" method="post" action="">
					...
					<div>
						<label for="name">'.$this->__L("YOURNAME").'</label>
						<input type="text" name="name" id="name" disabled="disabled" value="'.$this->commax_user_name.'" />
						<label for="email">'.$this->__L("YOUREMAIL").'</label>
						<input type="text" name="email" id="email" disabled="disabled" value="'.$this->commax_user_email.'" />
						<label for="url">'.$this->__L("YOURSITE").'</label>
						<input type="text" name="url" id="url" />
						<label for="body">'.$this->__L("YOURCOMM").'</label>
						<textarea name="body" id="body" cols="20" rows="5"></textarea>
	...
	*
	*/
	public function comment_closed_XHTML($ref_id) {
		$out = '
		<div id="addCommentContainer">
	...
	*
	*/
	public function comment_only_register_XHTML($ref_id) {
		$out = '
		<div id="addCommentContainer">
	...
	*
	*/
	public function reply_form_XHTML($id, $ref_id) {
		$out = '
		<div class="no_javascript_no_form">'.$this->__L("NOJSNOCOMM").'</div>
		<div style="display: none;" class="yes_javascript_yes_form">
			<form id="addCommentReply_'.$id.'" class="addReplyForm" method="post" action="">
				'.$this->__L("ADDREPLY").'
				<input type="hidden" name="ref_id" id="reply_ref_id_'.$id.'" value="'.$ref_id.'" />
				<input type="hidden" name="parent" value="'.$id.'" />
				<div>
					<label for="reply_name_'.$id.'">'.$this->__L("YOURNAME").'</label><br>
					<input type="text" name="name" id="reply_name_'.$id.'" />

					<label for="reply_email_'.$id.'">'.$this->__L("YOUREMAIL").'</label><br/>
					<input type="text" name="email" id="reply_email_'.$id.'" />

					<label for="reply_url_'.$id.'">'.$this->__L("YOURSITE").'</label><br />
					<input type="text" name="url" id="reply_url_'.$id.'" />

					<label for="reply_body_'.$id.'">'.$this->__L("YOURCOMM").'</label><br/>
		...
		if ($this->captcha_enabled == true) {
			$out .= '
					<img src="'.$this->commax_url.'include/commax_captcha.php" class="commax_captcha_img commax_captcha_img_repl cci_rpl_'.$id.'" />
					<a href="#" id="cgi_rpl_'.$id.'" class="commax_change_image_repl">'.$this->__L("CHANGECAPTCHA").'</a>
					<label for="commax_anti_bot_input_'.$id.'">'.$this->__L("INSERTCAPTCHA").'</label>
					<input type="text" id="commax_anti_bot_input_'.$id.'" name="commax_anti_bot_input" class="commax_anti_bot_input" />
					';
		}
	...
	*
	*/
	public function reply_form_user_logged_in_XHTML($id, $ref_id) {
		$out = '
		<div class="no_javascript_no_form">'.$this->__L("NOJSNOCOMM").'</div>
		<div style="display: none;" class="yes_javascript_yes_form">
			<form id="addCommentReply_'.$id.'" class="addReplyForm" method="post" action="">
				...
				<div>
					<label for="reply_name_'.$id.'">'.$this->__L("YOURNAME").'</label><br>
					<input type="text" name="name" id="reply_name_'.$id.'" disabled="disabled" value="'.$this->commax_user_name.'" />

					<label for="reply_email_'.$id.'">'.$this->__L("YOUREMAIL").'</label><br/>
					<input type="text" name="email" id="reply_email_'.$id.'" disabled="disabled" value="'.$this->commax_user_email.'" />
	...
	*/
	private function karma_increaseVote($ID) {
		...
		mysqli_query("UPDATE commax_comments SET up_vote = up_vote + 1 WHERE id = '$ID'");
		// get the initial karma value, before adding new vote
		$result 		= mysqli_query("SELECT karma FROM commax_comments where id = '$ID'") or die("error db - com.class #" . __line__);
		$row 			= mysqli_fetch_object($result);
		$intial_karma 	= $row->karma;
		// adds one more vote
		$final_karma	= $intial_karma + 1;
		// updates karma with new vote
		mysqli_query("UPDATE commax_comments SET karma = '$final_karma' WHERE id = '$ID'");
	...
	*/
	private function karma_decreaseVote($ID) {
		...
		mysqli_query("UPDATE commax_comments SET dw_vote = dw_vote + 1 WHERE id = '$ID'");
		// get the initial karma value, before adding new vote
		$result 		= mysqli_query("SELECT karma FROM commax_comments where id = '$ID'") or die("error db - com.class #" . __line__);
		$row 			= mysqli_fetch_object($result);
		$intial_karma 	= $row->karma;
	...
	*/
	private function karma_checkAlreadyVoted($ID) {
		...
		if ($type == "cookie") {
			...
			$cookie_name = md5("Commax_karma_votes");
			// check if it exists
		...
		// Oh well... voting only for registered users.
			...
			// So get users email
			$email 	= $this->commax_user_email;
			// check if we have it's email in our database having in mind the actual comment or reply ID
			$result = mysqli_query("SELECT id FROM commax_karma_voted WHERE com_id = '$ID' AND email = '$email'");
			...
			mysqli_query("INSERT INTO commax_karma_voted(id, com_id, email) VALUES ('', '$ID', '$email')");
			// return false (User has not voted yet
			return false;
		}
	}
	...
	*/
	public function validate(&$arr, $postType, $parentID = "") {
		...
		if ($postType === "reply") {
			$field_ref_id 	= "reply_ref_id_".$parentID;
			...
			$field_captcha 	= "commax_anti_bot_input_".$parentID;
		} else {
			$field_ref_id 	= "ref_id";
			$field_parent 	= "parent";
			$field_email 	= "email";
			...
			$field_body 	= "body";
			$field_name 	= "name";
			$field_captcha 	= "commax_anti_bot_input";
		}

		// Using the filter_input function introduced in PHP 5.2.0
		if (!($data['ref_id'] = filter_input(INPUT_POST,'ref_id',FILTER_SANITIZE_STRING))) {
			$errors["$field_ref_id"] = $this->__L("REFIDPROB");
		}
		...
		// Using the filter_input function introduced in PHP 5.2.0
		// but wait. Check if our system is running in registered users only. If true, the field email will come empty and then we MUST use our email variable $this->commax_user_email
		if ($this->reg_users_only == false) {
			if (!($data['email'] = filter_input(INPUT_POST,'email',FILTER_VALIDATE_EMAIL))) {
				$errors["$field_email"] = $this->__L("VALIDEMAIL");
			}
		} else {
			if ((!$data['email'] = filter_var($this->commax_user_email,FILTER_VALIDATE_EMAIL))) {
		...
		// Again check if our system is running in registered users only. If true, the field name will come empty and then we MUST use our user name variable $this->commax_user_name
		if ($this->reg_users_only == false) {
			...
			if (!($data['name'] = filter_var($this->commax_user_name,FILTER_SANITIZE_STRING))) {
		...
		if (($this->captcha_enabled == true) and ($this->reg_users_only == false)) {
			...
			if (md5($_POST['commax_anti_bot_input']) !== $_SESSION['commax_random_number'])	{
	...
	* Important function. Also Public this one, called from the commax.processor.php
	...
	*/
	public function insert_new_comment(&$arr) {
		...
		mysqli_query("INSERT INTO commax_comments(ref_id, parent, name, url, email, body, show_it)
						VALUES (
							'".$arr['ref_id']."',
							'".$arr['parent']."',
							'".$arr['name']."',
	...
	*/
    ...
    public function display_items_per_page() {
        ...
        return "<span class=\"paginate\">Items per page:</span><select class=\"paginate\" onchange=\"window.location='$_SERVER[PHP_SELF]?coms_page=1&ipp='+this[this.selectedIndex].value;return false\">$items</select>\n";
    }
    ...
    public function display_jump_menu() {
        ...
        return "<span class=\"paginate\">Page:</span><select class=\"paginate\" onchange=\"window.location='$_SERVER[PHP_SELF]?coms_page='+this[this.selectedIndex].value+'&ipp=$this->items_per_page';return false\">$option</select>\n";
    }
	...
	*/
    public function display_pages($extra_class = "") {
        return '<div class="commax_paginator '.$extra_class.'"><span class="commax_paginator_title">'. $this->__L("PAGES") .':</span> ' . $this->return . '<br clear="all" /></div>';
    }
	...
	*/
	public function getConfigurations($wich = "all") {
		...
		// get result
		$result = mysqli_query("SELECT * FROM commax_config $where_clause ORDER BY id ASC");
	...
	*/
	public function removeComment($id) {
		...
		mysqli_query("DELETE FROM commax_comments WHERE id = '$id'");
	...
	*/
	public function approveComment($id) {
		...
		// change status in db
		mysqli_query("UPDATE commax_comments SET show_it = '1' WHERE id = '$id'");
	...
	*/
	public function saveConfiguration($wich, $what) {
		return mysqli_query("UPDATE commax_config SET cfg_value = '$what' WHERE cfg_name = '$wich'") or die("HMMMM");
	}
	...
	*/
	public function adminGetMainComments() {
		$result = mysqli_query("SELECT ref_id, COUNT(id) AS total FROM commax_comments GROUP BY ref_id");
		return $result;
	}
	...
	*
	*/
	public function adminGetUnreviewdTotalComments($ref_id) {
		$result = mysqli_query("SELECT COUNT(id) AS total FROM commax_comments WHERE ref_id = '$ref_id' AND show_it = '0'");
		while($row = mysqli_fetch_assoc($result)) {
			$out = $row["total"];
		}
		return $out;
	...
	*
	*/
	public function admin_build_comment_system($ref_id) {
	...
	*
	*/
	public function admin_build_unreviewed_comment_system($ref_id) {
		$this->admin_viewing 			= true;
		$this->admin_viewing_backend 	= true;
		$out = $this->recursive_build_unreviewed_comment($ref_id);
		return $out;
	}
	...
	*
	*/
	private function recursive_build_unreviewed_comment($ref_id) {

		$and_show_it = "AND show_it = '0'";

		$result 	= mysqli_query("SELECT * FROM commax_comments WHERE ref_id = '$ref_id' $and_show_it ORDER BY id ASC");
		$out 		= "";
	...
	*/
	public function admin_approveComment($id) {
		if (mysqli_query("UPDATE commax_comments SET show_it = '1' WHERE id = '$id'"))
			return "1";
		else
			return "Probem in commax.class (#" . __LINE__ . ")";
	}
	...
	*/
	public function admin_removeComment($id) {
		...
		else if (mysqli_query("DELETE FROM commax_comments WHERE id = '$id'"))
			return "1";
